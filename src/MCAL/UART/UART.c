...
*/
#define UDRE_INTERRUPT_ON()  set_bits(_UCSRA, _UDRIE);
#define UDRE_INTERRUPT_OFF() clr_bits(_UCSRA, _UDRIE);
...
void UART_Init(uint32 Baudrate){
    ...
    uint16 UBBR = F_CPU/(16*Baudrate) - 1;
...
void UART_ReadString(uint8* out_str, uint8 delimitter){
    uint8 i=0;
    while (CB_front(&RX_Buffer) != delimitter){
